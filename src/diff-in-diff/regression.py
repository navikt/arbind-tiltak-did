...
One model specification is used — region FE + year-month FE — estimated via
OLS with explicit dummy variables (:func:`pandas.get_dummies`) and
:mod:`statsmodels`.  The distinction between "baseline" and "preferred" comes
from the input data:

* **Baseline**   – regular (non-flattened) panel
* **Preferred**  – seasonally flattened panel (flatten=True in data prep)
